...
魔法数（Magic Number，4字节）：
用于标识消息的类型和格式，可以用来快速识别和校验消息是否符合预期的格式。通常是一个固定的数字或者特定的模式，以便接收端能够快速判断消息是否正确。
版本（Version，1字节）：
表示消息协议的版本号。当消息格式发生变化时，可以通过版本号来进行兼容性检查，确保双方使用的协议版本一致。
消息长度（Message Length，4字节）：
指明消息体的总长度（不包括消息头的长度）。这对于接收方来说非常重要，因为它允许接收方知道何时一个完整的消息被接收到。
消息类型（Message Type，1字节）：
用于区分不同类型的消息，比如请求消息、响应消息、心跳消息等。通过消息类型可以快速定位消息的处理逻辑。
压缩类型（Compression Type，1字节）：
指示消息体是否经过压缩，以及使用的压缩算法（如gzip、deflate等）。这有助于接收方正确解压消息体。
序列化类型（Serialization Type，1字节）：
指定消息体的序列化方式，比如JSON、Protobuf、XML等。接收方可以根据此信息正确地反序列化消息体。
请求ID（Request ID，4字节）：
用于唯一标识一次请求，特别是在有多个请求同时发生时，可以帮助关联请求和响应。这对于调试和追踪请求非常有用。
